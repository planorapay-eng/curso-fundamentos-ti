# 💻 Curso: Fundamentos de TI — Hardware e Software

Curso interativo em HTML/CSS/JS puro, feito para **iniciantes totais** em tecnologia.
Carga estimada: **~31 horas** · **10 módulos** · **50 questões de quiz** com correção instantânea.

## 🚀 Como usar

Basta abrir o arquivo `index.html` em qualquer navegador — não precisa instalar nada.

Para rodar como site local (opcional):

```bash
python3 -m http.server 8000
# depois acesse http://localhost:8000
```

## 📚 Módulos

| # | Módulo | Carga |
|---|--------|-------|
| 1 | [Bem-vindo ao mundo da TI: hardware vs. software](modulo-01.html) | 2h |
| 2 | [Por dentro do computador: componentes principais](modulo-02.html) | 3h |
| 3 | [Processador e memória: CPU e RAM](modulo-03.html) | 3h |
| 4 | [Armazenamento: HD, SSD e nuvem](modulo-04.html) | 3h |
| 5 | [Periféricos, portas e conexões](modulo-05.html) | 2h |
| 6 | [Software e sistemas operacionais](modulo-06.html) | 3h |
| 7 | [Windows, Linux e macOS](modulo-07.html) | 3h |
| 8 | [Arquivos e pastas: organização e formatos](modulo-08.html) | 3h |
| 9 | [Redes e internet](modulo-09.html) | 4h |
| 10 | [Segurança, manutenção e projeto final](modulo-10.html) | 4h |

## ✨ Recursos

- ✅ Quiz com 5 questões por módulo, correção instantânea e explicações
- 📈 Barra de progresso salva no navegador (localStorage)
- 🛠️ Atividades práticas em cada módulo
- 🧠 Analogias didáticas (a "cozinha do computador")
- 🏆 Projeto final: diagnóstico completo do seu dispositivo
- 🌙 Visual dark, responsivo, sem dependências externas

## 📁 Estrutura

```
├── index.html          # Página inicial do curso
├── modulo-01.html ... modulo-10.html
└── assets/
    ├── style.css       # Estilos
    └── main.js         # Quiz + progresso
```

## 🌐 Publicar grátis no GitHub Pages

1. Vá em **Settings → Pages** do repositório
2. Em *Source*, escolha **Deploy from a branch** → `main` / `/ (root)`
3. Salve — em ~1 minuto o curso estará no ar em `https://SEU-USUARIO.github.io/NOME-DO-REPO`

---

Feito com fins educacionais. Sinta-se livre para usar, adaptar e compartilhar. 📖
